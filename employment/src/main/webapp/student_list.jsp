                    ...
                    <table class="layui-table layui-form">
                        ...
                        <c:forEach items="${studentList}" var="student">
                            <tr>
                                ...
                                <td>
                                    <c:out value="${student.spwd}"/>
                                </td>
                                <td>
                                    <c:out value="${student.sidcard}"/>
                                </td>
                                <td>
                                ...
                                <td >
                                    <c:out value="${student.sphone}"/>
                                </td>
                                <td >
                                    <c:out value="${student.semail}"/>
                                </td>
                                <td >
                                    <c:out value="${student.smid.mname}"/>
                                </td>
                                <td >
                                    <c:out value="${student.sbgdate}"/>
                                </td>
                                <td >
                                    <c:out value="${student.seducation}"/>
                                </td>
                                <td >
                                    <c:if test="${student.sflag == 0}">
                                        未就业
                                    </c:if>
                                    <c:if test="${student.sflag == 1}">
                                        求职中
                                    </c:if>
                                    <c:if test="${student.sflag == 2}">
                                        已就业
                                    </c:if>
                                    <c:if test="${student.sflag == 3}">
                                        离职
                                    </c:if>
                                    <c:if test="${student.sflag == 4}">
                                        再次就业
                                    </c:if>
                                </td>
                                    <td class="td-manage">
                                        <button class="layui-btn layui-btn layui-btn-xs"
                                                onclick="xadmin.open('修改','${pageContext.request.contextPath}/student/studentById?sid=${student.sid}',700,500)" >
                                            <i class="layui-icon">&#xe642;</i>修改</button>
                                        <button class="layui-btn-danger layui-btn layui-btn-xs"
                                                onclick="member_del(this,'${student.sid}')" href="javascript:;" >
...
<script>
    ...
    function member_del(obj,id){
        layer.confirm('确认要删除吗？',{icon:3,title:'提示信息'},function(index){
            $.ajax({
                type:"POST",
                url:"${pageContext.request.contextPath}/student/studentDel",
                dataType:"text",
                data: {sid: id},
